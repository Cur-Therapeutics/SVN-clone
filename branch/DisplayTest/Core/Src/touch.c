	...
	switch (mTouchState)
	{
		...
		case eTOUCH_DEBUG:
			...
			if (touchDetected == GPIO_PIN_RESET)
			{
				...
				//LCD_Draw(mLastXTouch, mLastYTouch);
			}
...
uint16_t TOUCH_Read(uint8_t cmd)
{
	...
	retval |= HAL_I2C_Master_Transmit(&hi2c1, TOUCH_SLAVE_ADDRESS, &cmd, 1, TOUCH_TIMEOUT);
	if (retval != HAL_OK)
	{
	...
	uint16_t result;
	uint8_t readData[2];
	retval |= HAL_I2C_Master_Receive(&hi2c1, TOUCH_SLAVE_ADDRESS + 1, (uint8_t*)readData, 2, TOUCH_TIMEOUT);
	if (retval != HAL_OK)
	{
		FaultHandler(ERR_TOUCH_COMMS);
		HealthSubsystemBad(eSystemTouch);
