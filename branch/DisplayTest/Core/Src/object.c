...
void ObjectDraw(sDisplayObject obj, int8_t windowOffset, int16_t xOffset, int16_t yOffset)
{
	...
	// Adjust destination and background coords per screen rotation
	uint16_t temp = obj.dst.xPos;
	obj.dst.xPos = DISPLAY_WIDTH - obj.dst.yPos - obj.shape.width;
	obj.dst.yPos = temp;

	temp = obj.bkg.xPos;
	obj.bkg.xPos = DISPLAY_WIDTH - obj.bkg.yPos - obj.shape.width;
	obj.bkg.yPos = temp;

		// Adjust width if off screen
	if (obj.dst.xPos >= DISPLAY_WIDTH)
	{
		return;
	}

	if (obj.dst.yPos >= DISPLAY_HEIGHT)
	{
		return;
	}

	if (obj.dst.xPos + obj.shape.width > DISPLAY_WIDTH)
	{
		obj.shape.width = DISPLAY_WIDTH - obj.dst.xPos;
	}

	if (obj.dst.yPos + obj.shape.height > DISPLAY_HEIGHT)
	{
		obj.shape.height = DISPLAY_HEIGHT - obj.dst.yPos;
	}
...
void ObjectDrawBlend(sDisplayObject obj, int8_t windowOffset, int16_t xOffset, int16_t yOffset)
{
	...
	if (obj.bkg.xPos + obj.shape.width > DISPLAY_WIDTH)
	{
		obj.shape.width = DISPLAY_WIDTH - obj.bkg.xPos;
	}

	if (obj.bkg.yPos + obj.shape.height > DISPLAY_HEIGHT)
	{
		obj.shape.height = DISPLAY_HEIGHT - obj.bkg.yPos;
	}
...
{
	sColor color = {0,0,0};
	LCD_FillLocation(obj.dst, obj.shape, color, 0);
}
